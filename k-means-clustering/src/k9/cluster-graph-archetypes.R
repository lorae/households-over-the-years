...
k_data <- readRDS("k-means-clustering/throughput/k9-best-model-cluster-labels.rds")

# ----- Step 4: Get cluster counts by years ----- #
clusters_raw <- k_data |>
  mutate(
    cluster_name = case_when(
      cluster == 1 ~ "Small grandfamily",
      cluster == 2 ~ "Some roommates",
      cluster == 3 ~ "Nuclear family",
      cluster == 4 ~ "Single person",
      cluster == 5 ~ "Extended family",
      cluster == 6 ~ "Large grandfamily",
      cluster == 7 ~ "Adult child + parent",
      cluster == 8 ~ "Few roommates",
      cluster == 9 ~ "Many roommates"
    )
  )
...
cluster_counts_year <- crosstab_count(
  ...
  tidyr::complete(
    YEAR = c(seq(1900, 2000, by = 10), 2006, 2011,2016, 2021, 2023), 
    fill = list(weighed_count = NA))
...
plot_cluster_trends <- function(data, y_var, y_label, title, palette = "Set3") {
...
ggsave("k-means-clustering/output/figures/fig12-96-household-counts-time-line.jpg",
       plot_counts, 
       width = 6, height = 6, 
       dpi = 300,
       scale = 1.25)

ggsave("k-means-clustering/output/figures/fig13-k9-household-percents-time-line.jpg",
       plot_percents, 
       width = 6, height = 6, 
       dpi = 300,
       scale = 1.25)

ggsave("k-means-clustering/output/figures/fig14-k9-household-relative-1900-line.jpg",
       plot_relative1900, 
       width = 6, height = 6, 
       dpi = 300,
       scale = 1.25)

ggsave("k-means-clustering/output/figures/fig15-k9-household-relative-2023-line.jpg",
       plot_relative2023, 
       width = 6, height = 6, 
       dpi = 300,
       scale = 1.25)
...
write_csv(cluster_counts_year,
          "k-means-clustering/output/figure-data/fig12-k9-household-counts-time-line.csv")

write_csv(cluster_percents_year,
          "k-means-clustering/output/figure-data/fig13-k9-household-percents-time-line.csv")

write_csv(cluster_percents_rel_1900,
          "k-means-clustering/output/figure-data/fig14-k9-household-relative-1900-line.csv")

write_csv(cluster_percents_rel_2023,
          "k-means-clustering/output/figure-data/fig15-k9-household-relative-2023-line.csv")
