...
fig06_data_path <- paste0("k-means-clustering/output/figure-data/fig06-k6-model-ss-", timestamp, ".csv")
fit_results |>
  select(-model) |>
...
fig06_path <- paste0("k-means-clustering/output/figures/fig06-k6-model-ss-", timestamp, ".png")
...
best_model_path <- "k-means-clustering/throughput/k6-best-model.rds"
saveRDS(best_model, best_model_path)
message("✅ Best-fit model saved to: ", best_model_path)

# Raw data with cluster labels
labelled_cluster_path <- "k-means-clustering/throughput/k6-best-model-cluster-labels.rds"
...
# All models
all_models_path <- paste0("k-means-clustering/throughput/k6-all-models-", timestamp, ".rds")
